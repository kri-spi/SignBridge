...
export default function GestureCamera({ onStatusChange }: GestureCameraProps) {
  ...
  const handleCameraReady = useCallback(() => {
    // Add a delay to ensure video stream has frames
    setTimeout(() => {
      if (isMountedRef.current) {
        setIsCameraReady(true);
        console.log('✅ Camera is ready');
      }
    }, 500); // Wait 500ms for video stream to stabilize
  }, []);
...
const captureFrame = useCallback(async () => {
  ...
  if (!cameraRef.current || !isMountedRef.current || !isCameraReady) {
    console.log('⏸️ Skipping capture - one condition failed');
    return;
  }
  ...
  try {
    ...
    if (error instanceof Error && error.message?.includes('enough camera data')) {
      console.log('⏳ Waiting for camera data...');
      return;
    }
...
}, [cameraRef, isMountedRef, isCameraReady, sendFrame]);
  ...
  }, [status, isCapturing, isCameraReady, currentSign, confidence, landmarks, permission]);
  ...
  useEffect(() => {
    if (isCapturing && status === "connected" && isCameraReady) {
      ...
      console.log('⏹️ Stopping frame capture:', { isCapturing, status, isCameraReady });
  ...
  useEffect(() => {
    if (permission?.granted) {
      setIsCapturing(true);
      console.log('📷 Camera permission granted, starting capture');
    }
  }, [permission?.granted]);
